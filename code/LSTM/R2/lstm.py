...
# 构建模型
def create_model(learning_rate=0.001, l2_penalty=0.1):
    model = Sequential()
    model.add(LSTM(80, input_shape=(SEQ_LEN,1), return_sequences=False,kernel_regularizer=l2(l2_penalty)))
    # model.add(Dropout(0.1))
    ##未归一化会导致训练集后期效果变差，模型过于复杂也会导致效果差
    # model.add(LSTM(3, return_sequences=False,kernel_regularizer=l2(l2_penalty)))
    # model.add(Dropout(0.1))
...
def predict_next(model, sample, epoch=12):  # 预测未来20个值
    temp1 = list(sample[:,0])
...
plt.figure(dpi=600)
#year_starts = [datetime(year, 1, 1) for year in range(x_all_date[0].year, x_all_date[-1].year + 1)]
...
text_date1 = datetime(2021, 11, 1)
text_date2 = datetime(2017, 12, 1)
plt.text(text_date1, -250, R_to_display, color='red')
# plt.text(text_date2, -5, '(a)', fontsize='medium', color='black')
plt.xlabel('Date')
plt.ylabel('Cumulative deformation (mm)')
plt.legend(frameon=False)
plt.subplots_adjust(left=0.12, right=0.95, top=0.98, bottom=0.1)
