...
future_dates = pd.date_range(start='2023-01-08', end='2120-01-01')
...
def find_nearest_date(target_value, start_date, model, popt):
    ...
    target = target_value
    # 定义日期范围
    search_dates = pd.date_range(start=start_date, end='2120-01-01')
    # 转换为距离2018年1月10日的天数
...
print("end_vel:",der_end)
# 绘制数据点
plt.rc('font',family='Times New Roman', size=11)
fig, axes = plt.subplots(2, 1,figsize=(8,7),dpi=600)
...
axes[0].text(nearest_date-timedelta(days=365*2), target_value+200, D_to_display, color='blue')
text_date1 = datetime(2100, 1, 1)
axes[0].text(text_date1, -900, T_to_display, color='blue')
...
axes[1].plot(t_fit_dates[5:], L_derivative[5:], color='black', zorder=1)
...
axes[1].text(nearest_date-timedelta(days=365*2), -30, D_to_display3, color='blue')
...
ax1.text(text_date2, -875, R_to_display, color='blue', fontsize=12)
